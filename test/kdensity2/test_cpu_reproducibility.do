...
*! Run: stata -b do test/kdensity2/test_cpu_reproducibility.do
...
display as text "kdensity2 CPU Reproducibility (1-core vs `max_cores'-core)"
...
set processors 1
forvalues i = 1/10 {
    kdensity2 x1, bw(silverman) generate(s1_`i')
}
local ok = 1
forvalues i = 1/9 {
    local j = `i' + 1
...
forvalues i = 1/10 {
    kdensity2 x1, bw(silverman) generate(s2_`i')
}
local ok = 1
forvalues i = 1/9 {
    local j = `i' + 1
...
forvalues i = 1/10 {
    set seed 123
    kdensity2 x1, bw(cv) generate(c1_`i')
}
local ok = 1
forvalues i = 1/9 {
    local j = `i' + 1
...
forvalues i = 1/10 {
    set seed 123
    kdensity2 x1, bw(cv) generate(c2_`i')
}
local ok = 1
forvalues i = 1/9 {
    local j = `i' + 1
...
kdensity2 x, bw(silverman) generate(f1)
local t1 = clock(c(current_time), "hms")
local t1_s = (`t1' - `t0') / 1000

set processors `max_cores'
local t0 = clock(c(current_time), "hms")
kdensity2 x, bw(silverman) generate(f16)
local t1 = clock(c(current_time), "hms")
...
set seed 456
local t0 = clock(c(current_time), "hms")
kdensity2 x, bw(cv) generate(f1_cv)
local t1 = clock(c(current_time), "hms")
...
set seed 456
local t0 = clock(c(current_time), "hms")
kdensity2 x, bw(cv) generate(f16_cv)
local t1 = clock(c(current_time), "hms")
