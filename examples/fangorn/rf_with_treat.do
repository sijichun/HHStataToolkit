...
fangorn y x g, type(regress) target(treatment) gen(conterfacurals) if(csa==1) mermaid("rf_with_treat_mermaid.md")
...
(scatter conterfacurals_pred x if csa==1 & treatment==1), legend(order(1 "Treatment, Group 0" 2 "Control, Group 0" 3 "Treatment, Group 1" 4 "Control, Group 1" 5 "Counterfactuals"))
graph export "scatter_plot2.png", replace

fangorn y x g, type(regress) ntree(100) target(treatment) gen(conterfacurals_rf) if(csa==1)
...
(scatter y x if treatment==1 & g==0) ///
(scatter y x if treatment==1 & g==1), legend(order(1 "Control, Group 0" 2 "Control, Group 1" 3 "Treatment, Group 0" 4 "Treatment, Group 1")) 
graph export "scatter_plot3.png", replace
twoway (scatter y x if csa==1 & treatment==1 & g==0) ///
(scatter y x if csa==1 & treatment==0 & g==0) ///
(scatter y x if csa==1 & treatment==1 & g==1) ///
(scatter y x if csa==1 & treatment==0 & g==1) ///
(scatter conterfacurals_rf_pred x if csa==1 & treatment==1), legend(order(1 "Treatment, Group 0" 2 "Control, Group 0" 3 "Treatment, Group 1" 4 "Control, Group 1" 5 "Counterfactuals"))
graph export "scatter_plot4.png", replace