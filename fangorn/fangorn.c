...
STDLL stata_call(int argc, char *argv[])
{
    ...
    if (entcvdepth >= 2 && n_train >= entcvdepth * 2) {
        /* Make train_idx copy and shuffle for fold assignment */
        int *cv_idx = (int *)malloc((size_t)n_train * sizeof(int));
        if (cv_idx) {
            memcpy(cv_idx, train_idx, (size_t)n_train * sizeof(int));
            ...
            /* Assign folds */
            int *fold_of = (int *)malloc((size_t)n_train * sizeof(int));
            if (fold_of) {
                for (i = 0; i < n_train; i++) fold_of[i] = i % entcvdepth;
                for (i = n_train - 1; i > 0; i--) {
                    int j = (int)(lcg_uniform(&rng) * (i + 1));
                    int tmp = fold_of[i];
                    fold_of[i] = fold_of[j];
                ...
                if (!cv_scores) { free(fold_of); free(cv_idx); return 1; }
                {
                    int di;
                    ...
                    for (di = 1; di <= depth_upper; di++) {
                        ...
                        for (fi = 0; fi < entcvdepth; fi++) {
                            ...
                            build_tree(cv_tree, &data, &cv_params, cv_train, n_cv_train);
    ...
    build_tree(tree, &data, &params, train_idx, n_train);
    ...
    /* -----------------------------------------------------------------------
     * Write predictions back to Stata for all touse observations
     * ----------------------------------------------------------------------- */
    for (i = 0; i < n_obs; i++) {
